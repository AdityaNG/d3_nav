...
@torch.no_grad()
def main():
    # ckpt = 'checkpoints/d3nav/d3nav-epoch-06-val_loss-0.6668.ckpt'  # 2 layers, starts from traj_quantizer/d3nav-traj-epoch-132-val_loss-0.2792.ckpt
    # Gives crooked trajectories, probably because the GPT has to fit into the planner's embedding space and might have found a suboptimal representation
    
    # ckpt = 'checkpoints/d3nav/d3nav-epoch-01-val_loss-0.6826.ckpt'  # 3 layers, starts from d3nav/d3nav-epoch-06-val_loss-0.6668.ckpt
    # Increasing unfrozen GPT layers does not help
    
    # ckpt = 'checkpoints/d3nav/d3nav-epoch-01-val_loss-0.5547.ckpt'  # 2 layers, traj unfrozen, starts from d3nav/d3nav-epoch-06-val_loss-0.6668.ckpt
    # unfreezing the trajectory decoder unlocks a lot of performance. Model becomes very good at estimating speed, but many turns get missed, plus trajectory is still ever so slightly crooked
    
    ckpt = 'checkpoints/d3nav/d3nav-epoch-03-val_loss-0.5128.ckpt'  # 3 layers, traj unfrozen, starts from d3nav/d3nav-epoch-06-val_loss-0.6668.ckpt
    # Good checkpoint, l2 (1s) of 0.43
    ...
    for chunk in tqdm(range(num_chunks), desc="chunk"):
        ...
        for idx in tqdm(range(start_idx, end_idx), desc="frame"):
            ...
            y_tensor = torch.from_numpy(y).unsqueeze(0).to("cuda")  # Add batch dimension
            pred_trajectory = model(y_tensor)
            pred_trajectory = pred_trajectory[0].cpu().numpy()  # Remove batch dimension

            # 2. Process image through D3Nav model
            processed_img = torch.from_numpy(x).unsqueeze(0).to("cuda")  # Add batch dimension
            img_trajectory = model.model(processed_img)
            img_trajectory = img_trajectory[0].cpu().numpy()  # Remove batch dimension

            # Process original trajectory
            original_traj = y[:, [1, 2, 0]]
            original_traj[:,0] *= -1
            original_traj = np.vstack(([0, 0, 0], original_traj))  # Add origin point
            ...
            combined_bev = np.vstack([img_bev_orig, img_bev_recon, img_bev_d3nav])
            
            # Add labels
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(combined_cam, "Ground Truth", (10, 30), font, 1, (255, 255, 255), 2)
            cv2.putText(combined_cam, "Trajectory Quantizer Prediction", (10, combined_cam.shape[0]//3 + 30), font, 1, (255, 255, 255), 2)
            cv2.putText(combined_cam, "D3Nav Planner", (10, 2*combined_cam.shape[0]//3 + 30), font, 1, (255, 255, 255), 2)
            cv2.putText(combined_bev, "Ground Truth", (10, 30), font, 1, (255, 255, 255), 2)
            cv2.putText(combined_bev, "Trajectory Quantizer Prediction", (10, combined_bev.shape[0]//3 + 30), font, 1, (255, 255, 255), 2)
            cv2.putText(combined_bev, "D3Nav Planner", (10, 2*combined_bev.shape[0]//3 + 30), font, 1, (255, 255, 255), 2)
            ...
            if video_writer is None:
                ...
                print(f"Created new video file with dimensions {width}x{height}")
